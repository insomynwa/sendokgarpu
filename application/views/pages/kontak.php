<script>
	$(document).ready(function(){
		...
		$("#k-submit").click(function(){
			...
			var subjek ;
			var pesan = $("#k-pesan").val();//alert(subjek+" "+pesan+" "+nama+" "+email);
			if($("#k-subjek").val()=="other")
				subjek = $("#k-oth-subj").val();
			else
				subjek = $("#k-subjek").val();
			...
			$.ajax({
				...
				success:
					function(kontak) {
						if(kontak==false){
							$(".error-message").html("Mohon lengkapi field yang masih kosong.").fadeIn("slow");
						}else {
							$("#k-conf").html("<p>"+kontak+"</p>");
							$("#k-form").remove();
						}
					}
			});
