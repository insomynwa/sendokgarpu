			...
			buttons: [
				{
					text: "Ya",
					click: function() {
						if(content=='resep') { $.post( "index.php/delete-resep", { resep:id } ); }
						<?php if($this->session->userdata('user_id')==1): ?>
						if(content=='member') {
							$.post( 'index.php/delete-member', { member:id } ); }
						<?php endif; ?>
						loadContent(''+content);
						$(this).dialog("close");
					}
				...
				{
					text: "Tidak", click: function() { $(this).dialog("close"); }
				}
			]
			}); }
				...
				if(data.type=="member") {
					...
					for(i=0; i<(data.name).length; i++) {
						tbl_data = tbl_data+"<tr><td>"+(i+1)+"</td>\
											...
											<td>"+data.name[i]+"</td>\
											<td>"+data.email[i]+"</td>\
											<td><a class='delete' onclick='"+data.del_func[i]+"' id='"+data.id[i]+"' >"+data.del_txt[i]+"</a></tr>";
					}
				}
				<?php endif; ?>
