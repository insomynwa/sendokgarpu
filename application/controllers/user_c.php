...
class User_c extends CI_Controller {
	...
	public function update_profile() {
		...
		if(isset($_POST['akun-old-pass']) && isset($_POST['akun-new-pass']) && isset($_POST['akun-new-pass2'])) {
			...
			if($o_pass == '' || $n_pass == '' || $n2_pass == '') {
				...
				if($is_valid) {
					if($n_pass == $n2_pass) {
						...
						$output = array('status' => false, 'msg' => 'password baru tidak sesuai');
					}
				}else {
					$output = array('status' => false, 'msg' => 'password salah');
				}
			}
			...
			if($upl_img['status']) {
				$profil['photo'] = $upl_img['file'];
				$output = array('status' => true, 'msg' => 'foto profil berhasil diubah');
			}else {
				$output = array('status' => false, 'msg' => 'foto profil gagal diubah');
			}

			$this->membership->update_photo($user_id,$profil);
		}
	...
	private function _upload_img($filename) {
		...
		if ( ! $this->upload->do_upload('profil-gambar')){
